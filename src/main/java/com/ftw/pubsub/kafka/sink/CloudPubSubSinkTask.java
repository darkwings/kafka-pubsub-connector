...
public class CloudPubSubSinkTask extends SinkTask {
    ...
    public void put(Collection<SinkRecord> sinkRecords) {
        ...
        for (SinkRecord record : sinkRecords) {
            ...
            if (attributes.size() == 0 && value == null) {
                ...
                addFuture(record.topic(), record.kafkaPartition(), nullMessageFuture);
                continue;
            }
            ...
            if (orderingKeySource == CloudPubSubSinkConnector.OrderingKeySource.KEY && key != null && !key.isEmpty()) {
            ...
            addMessage(record.topic(), record.kafkaPartition(), message);
        }
    }
        ...
        for (Map.Entry<TopicPartition, OffsetAndMetadata> partitionOffset :
            ...
            try {
                ...
                if (toDlq(e)) {
                    ...
                    // Log is done at higher level
                    // WARNING: POSSIBILITY OF NO OFFSET COMMITTED: downstream consumer could receive duplicated messages
                    // for infinite times
    ...
    private void addMessage(String topic, Integer partition, PubsubMessage message) {
    ...
    private void addFuture(String topic, Integer partition, ApiFuture<String> future) {
        PubsubMessage message = PubsubMessage.newBuilder().build(); // An empty message
    ...
    private void createPublisher() {
        ...
        if (orderingKeySource != CloudPubSubSinkConnector.OrderingKeySource.NONE) {
