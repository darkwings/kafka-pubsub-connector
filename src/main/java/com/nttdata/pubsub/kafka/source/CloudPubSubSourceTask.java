...
public class CloudPubSubSourceTask extends SourceTask {
  ...
  private boolean forwardAsPlainString;
  ...
  public void start(Map<String, String> props) {
    ...
    kafkaPartitionScheme =
        CloudPubSubSourceConnector.PartitionScheme.getEnum(
    ...
    forwardAsPlainString = (Boolean) validatedProps.get(CloudPubSubSourceConnector.CPS_AS_PLAIN_STRING);
      ...
      if (useStreamingPull) {
        subscriber = new StreamingPullSubscriber(
            receiver -> {
              Subscriber.Builder builder = Subscriber.newBuilder(cpsSubscription, receiver)
                  ...
                  .setExecutorProvider(FixedExecutorProvider.create(ConnectorUtils.getSystemExecutor()));
        ...
        subscriber = new AckBatchingSubscriber(
            new CloudPubSubRoundRobinSubscriber(NUM_CPS_SUBSCRIBERS,
                ...
                cpsEndpoint, cpsSubscription, cpsMaxBatchSize), runnable -> ConnectorUtils.getSystemExecutor()
  ...
  public List<SourceRecord> poll() {
    ...
    try {
      ...
      for (ReceivedMessage rm : response) {
        ...
        SourceRecord record;
        if (forwardAsPlainString) {
          record = managePlainStringMessage(message, messageAttributes, key,
                  orderingKey, timestamp, messageData, ack);
        }
        else {
          record = manageBytesMessage(message, messageAttributes, key,
                  orderingKey, timestamp, messageData, ack);
        }
  ...
  private SourceRecord manageBytesMessage(PubsubMessage message, Map<String, String> messageAttributes,
                                          String key, String orderingKey, Long timestamp,
                                          ByteString messageData, Map<String, String> ack) {
    SourceRecord record;
  ...
  private SourceRecord managePlainStringMessage(PubsubMessage message, Map<String, String> messageAttributes,
                                                String key, String orderingKey, Long timestamp,
                                                ByteString messageData, Map<String, String> ack) {
    SourceRecord record;
    String messageContent = new String(messageData.toByteArray());
    if (useKafkaHeaders) {
      ConnectHeaders headers = prepareHeaders(messageAttributes, orderingKey);
  ...
  private ConnectHeaders prepareHeaders(Map<String, String> messageAttributes, String orderingKey) {
  ...
  private SourceRecord createRecordWithStruct(
    ...
    valueSchemaBuilder.field(ConnectorUtils.KAFKA_MESSAGE_CPS_BODY_FIELD, Schema.BYTES_SCHEMA);
    for (Entry<String, String> attribute :
        messageAttributes.entrySet()) {
      if (!attribute.getKey().equals(kafkaMessageKeyAttribute)) {
  ...
  private Integer selectPartition(Object key, Object value, String orderingKey) {
    if (kafkaPartitionScheme.equals(CloudPubSubSourceConnector.PartitionScheme.HASH_KEY)) {
      return key == null ? 0 : Math.abs(key.hashCode()) % kafkaPartitions;
    } else if (kafkaPartitionScheme.equals(CloudPubSubSourceConnector.PartitionScheme.HASH_VALUE)) {
      return Math.abs(value.hashCode()) % kafkaPartitions;
    } else if (kafkaPartitionScheme.equals(CloudPubSubSourceConnector.PartitionScheme.KAFKA_PARTITIONER)) {
      return null;
    } else if (kafkaPartitionScheme.equals(CloudPubSubSourceConnector.PartitionScheme.ORDERING_KEY) && orderingKey != null &&
        !orderingKey.isEmpty()) {
